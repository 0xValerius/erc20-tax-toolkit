...
contract TaxHandledTokenTest is Test {
    ...
    function test_MockTokenDeploy() public {
        ...
        assertEq(token.transferFeesBPs(), transferFee);
        assertEq(token.buyFeesBPs(), buyFee);
        assertEq(token.sellFeesBPs(), sellFee);
    ...
    function test_setFeeRate() public {
        ...
        vm.prank(actor1);
        vm.expectRevert("Ownable: caller is not the owner");
        token.setTransferFeesBPs(200);

        vm.prank(owner);
        token.setTransferFeesBPs(200);
        assertEq(token.transferFeesBPs(), 200);
    }
